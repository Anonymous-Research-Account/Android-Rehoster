...
<<<<<<< HEAD
ReHoster is designed to facilitate Android firmware analysis and emulation at scale. It provides tools for:

=======
## Overview

One part of the tooling is designed to facilitate Android firmware analysis and emulation at scale. It provides tools for:
...
- **Android Emulators**: Run in Docker containers with full Android system images
- **Coturn Server**: Provides STUN/TURN services for WebRTC connections
- **AOSP Build Tools**: Scripts for building and cusANONizing Android system images
- **FirmwareDroid Integration**: Backend connection for firmware download and analysis
...
- **gRPC API Exposure**: Access emulator APIs through standardized gRPC interface
- **WebRTC Streaming**: Real-time audio/video streaming from emulators
- **AOSP Build Pipeline**: Complete toolchain for building cusANON Android images
- **Firmware Injection**: Inject firmware packages and APEXs into AOSP builds
- **Docker-based**: Easy deployment and isolation using containers
- **Dynamic Configuration**: Template-based configuration for flexible setups
...
- **Python**: 3.8 or higher
- **Git**: For repository management
- **AOSP Build Tools** (optional, for building cusANON images):
  - Java Development Kit (JDK) 11
  - Android SDK Platform Tools
  - Required build dependencies (see AOSP documentation)
...
# The file contains settings for WebRTC/TURN server
# Edit if you need to cusANONize TURN credentials and server URL
nano env/.env
```
...
### Building CusANON Emulator Images
...
The script will auANONatically build Docker images for your architecture.
...
3. Injects files into AOSP build structure
4. Handles APEX repackaging and signing
5. Builds cusANON Android system images
6. Uploads resulting images to Docker registry (nexus)
...
### CusANONizing Emulator Configuration
